...
public:
    ...
    SuperquadricParticle(double mass, double shapex, double shapey, double shapez, double x, double y, double z,
                         double quat1, double quat2, double quat3, double quat4, Eigen::MatrixXd rotationMatrix, double vx = 0, double vy = 0,
                         double vz = 0)
...
//        double c3 = 1 - 2.0 * (quat2 * quat2 + quat3 * quat3);
        ...
        double a1 = rotationMatrix(0,0);
        double b1 = rotationMatrix(0,1);
        double c1 = rotationMatrix(0,2);

        double a2 = rotationMatrix(1,0);
        double b2 = rotationMatrix(1,1);
        double c2 = rotationMatrix(1,2);

        double a3 = rotationMatrix(2,0);
        double b3 = rotationMatrix(2,1);
        double c3 = rotationMatrix(2,2);

        q(0,0) = (c1 * c1 - 1.0 / 3.0) * (3.0 / 2.0);
        q(0,1) = (c1 * c2) * (3.0 / 2.0);
        q(0,2) = (c1 * c3) * (3.0 / 2.0);

        q(1,0) = (c2 * c1) * (3.0 / 2.0);
        q(1,1) = (c2 * c2 - 1.0 / 3.0) * (3.0 / 2.0);
        q(1,2) = (c2 * c3) * (3.0 / 2.0);

        q(2,0) = (c3 * c1) * (3.0 / 2.0);
        q(2,1) = (c3 * c2) * (3.0 / 2.0);
        q(2,2) = (c3 * c3 - 1.0 / 3.0) * (3.0 / 2.0);
    ...
    //Qij matrix to calculate order parameter S
    Eigen::MatrixXd q = Eigen::MatrixXd(3,3);
};
